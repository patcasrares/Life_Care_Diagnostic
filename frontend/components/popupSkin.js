...
export default function PopupSkin({returnHref, firstName, lastName, verdict}) {
  const router = useRouter();
  var result = `The result for the patient ${firstName} ${lastName} is ` + "\n"
  result += readDiagnostic(String(verdict).charAt(0)) + " according to ResNet with a probability of over 50% \n";
  result += readDiagnostic(String(verdict).charAt(1)) + " according to Sequential with a probability of over 60%";

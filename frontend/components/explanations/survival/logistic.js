
export default function SurvivalLogistic() {
 
  return (
    <div className="mb-2 pt-2 pb-2 h-full w-full">
      <div class="relative text-gray-500 text-lg mb-3 flex-auto">
        You can find more about Logistic Regression
        <a className="ml-1 text-blue-500" target="_blank" href="https://www.mastersindatascience.org/learning/introduction-to-machine-learning-algorithms/logistic-regression/">
          Here
        </a>
      </div>
      <div class="relative text-gray-500 text-lg flex-auto">
        Logistic Regression was applied on a dataset about tumours and survival, Haberman dataset.
      </div>
      <div class="relative text-gray-500 text-lg flex-auto">
        You can look down to see a part of the cases from that database
      </div>
      <div class="relative text-gray-500 text-lg flex-auto">
        <img src="images/survivalData.JPG"/>
        Here are some histograms
        <img src="images/survivalHist.JPG"/>
      </div>
     
      <div className="relative text-gray-500 text-lg flex-auto">
        <img src="/images/grafic1.JPG"/>
        <img src="/images/grafic2.JPG"/>
        <img src="/images/grafic3.JPG"/>
        {/*<img src="/images/grafic4.JPG"/>
        <img src="/images/grafic5.JPG"/>*/}
      </div>
      <div class="font-bold relative text-gray-500 text-lg flex-auto">
        Cross-validation was used to train the model.
      </div>
      <div class="ml-3 flot-left text-left relative text-gray-500 text-lg flex-auto">
        The dataset was split in this way:
        <ul>
          <li>60% for training.</li>
          <li>20% for validation.</li>
          <li>20% for testing.</li>
        </ul>
        <div class="font-bold">We evaluated the model on the testing set using the next metrics:</div>
        <ul>
          <li>Accuracy.</li>
          <li>Precision.</li>
          <li>Recall.</li>
          <li>FMeasure.</li>
        </ul>
        We repeated the process and evaluated it each time. We obtained the following confidence intervals:
        <ul>
          <li class="font-bold">Accuracy - (0.662, 0.693).</li>
          <li class="font-bold">Precision - (0.755, 0.831).</li>
          <li class="font-bold">Recall - (0.768, 0.776).</li>
          <li class="font-bold">FMeasure - (0.759, 0.790).</li>
        </ul>
        <div className='h-10'/>
      </div>
    </div>
  );
}

...
import PopupSurvival from '../components/popupSurvival';
...
function App() {
  ...
  const makeDiagnostic = () => {
    ...
    axios.get(BASE_URL, {params: {age: age, nodes: nodes}}).then(response => {
      setVerdict(response.data)
      router.push(
        makeContextualHref({resource: "survival", action: "survival", verdict: response.data}), '/survivalChances', {shallow: true}
      );
    }).catch(error=>{
      console.log(error)
    })
  ...
  return (
    ...
    <div class="bg-blue-300 overflow-hidden flex flex-row min-h-screen bg-gray-100 text-gray-800">
      ...
      <main class="bg-blue-300 text-center items-center justify-center main h-screen overflow-hidden flex flex-col flex-grow -ml-64 md:ml-0 transition-all duration-150 ease-in">
        <div className="mb-10 text-lg font-bold">Survival Chances</div>
        <div class="rounded-3xl sm:w-full p-10 bg-white w-full max-w-lg">
          <div class="flex flex-wrap -mx-3 mb-6">
            ...
            <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
              ...
              <input onChange={(e)=>{setAge(e.target.value)}} class={"appearance-none block w-full bg-gray-200 text-gray-700 border rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white " + (age !=''? '' : ' border-red-500')} id="grid-first-name" type="text" placeholder="Jane"/>
              {age =='' && <p class="text-red-500 text-xs italic">Please fill out this field.</p>}
            </div>
            <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
              <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2">
                Patient's Nodes
              </label>
              <input onChange={(e)=>{setNodes(e.target.value)}} class={"appearance-none block w-full bg-gray-200 text-gray-700 border rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white " + (nodes !=''? '' : ' border-red-500')} id="grid-first-name" type="text" placeholder="Jane"/>
              {nodes =='' && <p class="text-red-500 text-xs italic">Please fill out this field.</p>}
            </div>
          </div>
        ...
        {(router.query.resource === "survival") &&
          <PopupSurvival verdict={verdict} firstName={firstName} lastName={lastName} returnHref={returnHref}/> 
        }
      </main>
    </div>
  );

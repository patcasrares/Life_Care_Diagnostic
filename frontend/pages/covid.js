...
function App() {
  ...
  const BASE_URL = "http://localhost:5000/sound"
  ...
  const makeAudio = (startRecording, stopRecording) => {
    if (!audio) {
      startRecording()
      setTimeout(()=>{
        stopRecording();
  ...
  return (
    ...
    <div class="bg-blue-300 overflow-hidden flex flex-row min-h-screen bg-gray-100 text-gray-800">
      ...
      <main class="bg-blue-300 text-center items-center justify-center main h-screen overflow-hidden flex flex-col flex-grow -ml-64 md:ml-0 transition-all duration-150 ease-in">
        <div className="mb-10 text-lg font-bold">Covid Diagnostic</div>
        <div class="rounded-3xl sm:w-full p-10 bg-white w-full max-w-lg">
          <div class="flex flex-wrap -mx-3 mb-6">
            <div class="w-full px-3">
              ...
              {verdict == '0' && 
                <label class="block uppercase tracking-wide text-red-500 text-xs font-bold mb-2" for="grid-password">
                  There are signs of covid!!!
                </label>
              }
              <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" for="grid-password">
                Insert Audio recording
              </label>
              <input class={"appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500 " + (sound? '' : ' border-red-500')} id="grid-password" onChange={(e)=>{setSound(e.target.files[0])}} type="file" placeholder="******************"/>
              {!sound && <p class="text-red-500 text-xs italic">Insert the recording here please</p>}
            </div>
          </div>
          ...
          <div className="flex">
            ...
            onChange={()=>{alert(1)}}
              ...
              onStop={(blobUrl, blob) => {send(blob)} }
              render={({ status, startRecording, stopRecording, mediaBlob, mediaBlobUrl }) => (
                <div className="ml-40 mt-4 center">
                  <div>{mediaBlob}</div>
                  <div className="ml-10 flex">
                    <button onClick={()=>{makeAudio(startRecording, stopRecording)}} className="mr-2">
                      {!audio && <FaMicrophone className="h-10"/>}
                      {audio && <FaMicrophoneSlash className="h-10"/>}
                    </button>
